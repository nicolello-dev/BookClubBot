...
async function onSprintEnd(prisma, id, channel) {
  ...
  // After five minutes, say how well the sprint went
  setTimeout(() => onPagesSubmit(prisma, id, channel), 5 * 60 * 1000);
}
...
async function onPagesSubmit(prisma, id, channel) {
  ...
  const members = sprint.participants.map((m) => `<@${m.id}>`).join(" ");
  channel.send(`Sprint ${id} has ended! Here are the results:\n${members}`);
  sprint.participants.forEach(async (p) => {
    const userSprint = await prisma.userSprint.findFirst({
      where: {
        userId_sprintId: {
          userId: p.id,
          sprintId: id,
    ...
    channel.send(
      `${p.username} read ${
        userSprint.pagesFinal - userSprint.pagesInitial
      } pages during this sprint.`
    );
  });
}
